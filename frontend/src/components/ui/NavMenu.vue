  ...
  <div class="relative inline-flex" ref="menuRef">
    <button 
      ...
      :class="isOpen 
        ...
        : 'text-slate-600 dark:text-slate-300 hover:bg-slate-50 hover:text-slate-900 dark:hover:bg-slate-800'"
    >
    ...
    <transition name="dropdown">
      <div
        v-if="isOpen"
        class="absolute left-0 top-full mt-1 w-60 bg-white dark:bg-slate-900 rounded-lg shadow-xl shadow-slate-200/50 dark:shadow-slate-950/50 border border-slate-100 dark:border-slate-800 z-[9999] overflow-hidden py-1 transition-all"
      >
        <div v-if="visibleItems.length === 0" class="px-4 py-3 text-xs text-slate-400 italic text-center">
          Nenhum item disponível
        </div>
        
        <div v-else class="flex flex-col p-1">
          <template v-for="(item, index) in visibleItems" :key="index">
            <hr v-if="item.divider" class="my-1 border-slate-100 dark:border-slate-800 mx-2">
            
            <a 
              v-else
              @click="handleNav(item.to)" 
              class="flex items-center px-3 py-2 rounded-md text-slate-600 dark:text-slate-300 hover:bg-slate-50 hover:text-brand-primary cursor-pointer transition-all duration-150 group/item"
            >
              <div class="w-6 h-6 flex items-center justify-center rounded bg-slate-50 dark:bg-slate-800 group-hover/item:bg-white mr-2.5 transition-colors">
