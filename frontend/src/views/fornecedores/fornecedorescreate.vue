      ...
      <form @submit.prevent="submitForm">
        <div class="form-grid">
          ...
          <div class="form-group col-span-6">
            ...
            <input
              class="form-input"
              v-model="fornecedor.telefone_loja"
            />
          </div>
          ...
          <div class="form-group col-span-6">
            ...
            <input
              class="form-input"
              v-model="fornecedor.vendedor_nome"
            />
          </div>
          ...
          <div class="form-group col-span-6">
            ...
            <input
              class="form-input"
              v-model="fornecedor.vendedor_contato"
            />
          </div>
          ...
          <div class="form-group col-span-3">
            ...
            <input
              type="number"
              class="form-input"
              v-model="fornecedor.dia_fechamento"
            />
          </div>
          ...
          <div class="form-group col-span-3">
            ...
            <input
              type="number"
              class="form-input"
              v-model="fornecedor.dia_vencimento"
            />
          </div>
...
import { maskCNPJ, onlyNumbers } from '@/utils/utils'
...
export default {
  ...
  methods: {
    ...
    async onCnpj(e) {
      ...
      const mascarado = maskCNPJ(e.target.value)
      this.fornecedor.cpf_cnpj = mascarado

      // apenas números para consulta
      const numeros = onlyNumbers(mascarado)
      if (numeros.length !== 14) return

      try {
        this.carregandoCnpj = true

        const data = await buscarCNPJ(numeros)
        if (!data) return

// IDENTIDADE
this.fornecedor.razao_social = data.nome || ''
this.fornecedor.nome_fantasia =
  data.fantasia || this.fornecedor.nome_fantasia

// CONTATO
this.fornecedor.email_loja = data.email || ''
this.fornecedor.telefone_loja = data.telefone || ''

// ENDEREÇO
this.fornecedor.cep = data.cep || ''
this.fornecedor.endereco = data.logradouro || ''
this.fornecedor.bairro = data.bairro || ''
this.fornecedor.cidade = data.municipio || ''
