...
function normalizarModulo(modulo, fallback = {}, idx = 0) {
  return {
    id: modulo?.id ?? fallback?.id,
    modulo_key: modulo?.modulo_key || fallback?.modulo_key || `MOD_${idx + 1}`,
    titulo: modulo?.titulo || fallback?.titulo || modulo?.modulo_key || fallback?.modulo_key || `Módulo ${idx + 1}`,
    texto: modulo?.texto ?? fallback?.texto ?? '',
    ordem: typeof modulo?.ordem === 'number'
      ? modulo.ordem
      : typeof fallback?.ordem === 'number'
        ? fallback.ordem
        : idx + 1,
  }
}

function mesclarContratoComPadrao(listaBackend = []) {
  const padrao = contratosDefaults()
  const mapaBackend = new Map(
    (Array.isArray(listaBackend) ? listaBackend : [])
      .filter((m) => m?.modulo_key)
      .map((m) => [String(m.modulo_key).toUpperCase(), m]),
  )

  const combinados = padrao.map((pad, idx) => {
    const backend = mapaBackend.get(String(pad.modulo_key).toUpperCase()) || {}
    return normalizarModulo(backend, pad, idx)
  })

  // Mantém módulos extras que existirem no banco e não estiverem no padrão local
  const keysPadrao = new Set(padrao.map((p) => String(p.modulo_key).toUpperCase()))
  const extras = (Array.isArray(listaBackend) ? listaBackend : [])
    .filter((m) => m?.modulo_key && !keysPadrao.has(String(m.modulo_key).toUpperCase()))
    .map((m, idx) => normalizarModulo(m, {}, combinados.length + idx))

  return [...combinados, ...extras]
}
...
async function carregarContrato() {
  ...
  try {
    ...
    let normalizados = mesclarContratoComPadrao(lista)
