...
ssh -i "$KEY_PATH" "$EC2_HOST" \
  "cd ~/acasa-erp/frontend && git pull && npm ci && npm run build && sudo rsync -a --delete --exclude 'erp' --exclude 'ponto' --exclude 'downloads' dist/ $REMOTE_WEB_DIR/ && sudo chown -R nginx:nginx $REMOTE_WEB_DIR && sudo nginx -t && sudo systemctl reload nginx"

echo "OK: Android + Web atualizados (sem Tauri)."
