...
ssh -i "$KEY_PATH" "$EC2_HOST" \
  "sudo mv /home/ec2-user/index.html $REMOTE_WEB_DIR/index.html && sudo chown nginx:nginx $REMOTE_WEB_DIR/index.html"

echo "==> Atualizando ERP Web (frontend dist)"
ssh -i "$KEY_PATH" "$EC2_HOST" \
  "cd ~/acasa-erp/frontend && git pull && npm ci && npm run build && sudo rm -rf $REMOTE_WEB_DIR/* && sudo cp -r dist/* $REMOTE_WEB_DIR/ && sudo chown -R nginx:nginx $REMOTE_WEB_DIR && sudo nginx -t && sudo systemctl reload nginx"
