      ...
      <div class="p-8 sm:p-10 flex flex-col items-center">
        ...
        <div v-else class="w-full space-y-6 animate-in">
          <div class="w-full text-center border-b border-slate-100 pb-3">
            ...
            <p v-if="!empresaCnpj" class="text-[9px] text-amber-600 mt-1">Preencha Configurações → Cadastro da Empresa no ERP</p>
          </div>
          ...
          <div v-if="registrosHojeOrdenados.length" class="space-y-3">
            ...
            <div v-if="ultimoRegistro" class="bg-white border border-slate-100 rounded-2xl p-4 space-y-3">
              ...
              <button
                ...
                class="w-full h-11 rounded-xl bg-[#25D366] text-white text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-[#128C7E] active:scale-[0.98] transition-all disabled:opacity-60"
              >
                <span v-if="loadingComprovante" class="animate-spin h-5 w-5 border-2 border-white/30 border-t-white rounded-full" />
                ...
                {{ loadingComprovante ? 'Gerando...' : 'Enviar comprovante por WhatsApp' }}
              </button>
            </div>
          </div>
        </div>
