...
export const SUBETAPAS_CATALOGO: SubetapaCatalogItem[] = [
  {
    key: 'CADASTRO',
    label: 'Cadastro',
    macroetapa: 'COMERCIAL',
    fluxos: ['MARCENARIA'],
    ordem: 5,
    proximaSubetapa: 'MEDIDA',
  },
  {
    key: 'MEDIDA',
    label: 'Medida',
  ...
  },
  {
    ...
    fluxos: ['MARCENARIA'],
    ordem: 40,
    proximaSubetapa: 'MEDIDA_FINA',
  },
  {
    key: 'MEDIDA_FINA',
    label: 'Medida fina',
    macroetapa: 'ENGENHARIA',
    fluxos: ['MARCENARIA'],
    ordem: 50,
    proximaSubetapa: 'PROJETO_TECNICO',
  },
  {
    key: 'PROJETO_TECNICO',
  ...
  },
  {
    ...
    fluxos: ['MARCENARIA'],
    ordem: 70,
    proximaSubetapa: 'ENTREGA',
  },
  {
    key: 'SERVICO_CORTE_FITA_BORDA',
    label: 'Serviço de Corte e Fita de Borda',
    macroetapa: 'FABRICA',
    fluxos: ['SERVICO_CORTE'],
    ordem: 75,
    terminal: true,
    proximaSubetapa: null,
  },
  {
    key: 'ENTREGA',
    label: 'Material carregado',
    macroetapa: 'LOGISTICA',
    fluxos: ['MARCENARIA'],
    ordem: 80,
    proximaSubetapa: 'MONTAGEM',
  },
  {
    key: 'MONTAGEM',
    label: 'Montagem',
...
export const SUBETAPA_CATALOGO_POR_KEY = SUBETAPAS_CATALOGO.reduce<
  Record<Subetapa, SubetapaCatalogItem>
>((acc, item) => {
  acc[item.key] = item;
  return acc;
}, {} as Record<Subetapa, SubetapaCatalogItem>);

const SUBETAPAS_PRODUCAO_ATIVA = new Set<Subetapa>([
  'PRODUCAO',
  'SERVICO_CORTE_FITA_BORDA',
]);

const SUBETAPAS_FINALIZACAO_VENDA = new Set<Subetapa>([
  'MONTAGEM',
  'GARANTIA',
  'ASSISTENCIA',
  'MANUTENCAO',
...
const SUBETAPAS_OBRA_VIGENTE = new Set<Subetapa>([
  'MEDIDA_FINA',
  'PROJETO_TECNICO',
  'PRODUCAO',
  'SERVICO_CORTE_FITA_BORDA',
...
export function normalizarExecucaoEtapa(valor?: string | null): ExecucaoEtapa | null {
  ...
  if ((EXECUCOES_ETAPA as readonly string[]).includes(key)) {
    return key as ExecucaoEtapa;
  }
  return null;
}
...
export function normalizarSubetapa(valor?: string | null): Subetapa | null {
  const key = String(valor || '').trim().toUpperCase();
  if ((SUBETAPAS as readonly string[]).includes(key)) {
    return key as Subetapa;
  }
  return null;
}

export function statusClienteAoAgendarPorSubetapa(params: {
  subetapa?: Subetapa | string | null;
}): string | null {
...
export function categoriaAgendaLojaPorSubetapa(params: {
  subetapa?: Subetapa | string | null;
}): string | null {
  const subetapa = String(params.subetapa || '').trim().toUpperCase();
  ...
  switch (subetapa) {
    ...
    case 'PROJETO_TECNICO':
      return 'AGUARDANDO_PROJETO_TECNICO';
    case 'PRODUCAO':
    case 'SERVICO_CORTE_FITA_BORDA':
      return 'PRODUCAO_EM_ANDAMENTO';
    case 'ENTREGA':
    case 'MONTAGEM':
      return 'MONTAGEM_CLIENTE_AGENDADA';
    case 'GARANTIA':
    case 'ASSISTENCIA':
    case 'MANUTENCAO':
      return subetapa;
...
export function categoriaAgendaFabricaPorSubetapa(params: {
  subetapa?: Subetapa | string | null;
  execucaoEtapa?: ExecucaoEtapa | string | null;
}): string | null {
  const subetapa = String(params.subetapa || '').trim().toUpperCase();
  ...
  switch (subetapa) {
    ...
    case 'FECHAMENTO':
      return 'CONTRATO';
    case 'MEDIDA_FINA':
      return execucao === 'PENDENTE' ? 'AGENDAR_MEDIDA_FINA' : 'MEDIDA_FINA';
    case 'PROJETO_TECNICO':
      return 'PROJETO_TECNICO_EM_ANDAMENTO';
    case 'PRODUCAO':
    case 'SERVICO_CORTE_FITA_BORDA':
      return 'PRODUCAO_EM_ANDAMENTO';
    case 'ENTREGA':
...
export function statusClienteAoConcluirSubetapa(params: {
  subetapa?: Subetapa | string | null;
}): string | null {
  const subetapa = String(params.subetapa || '').trim().toUpperCase();
  ...
  switch (subetapa) {
    case 'MEDIDA':
      return 'MEDIDA_REALIZADA';
    case 'APRESENTACAO':
      return 'ORCAMENTO_APRESENTADO';
    case 'MEDIDA_FINA':
      return 'AGUARDANDO_PRECIFICACAO';
    case 'PROJETO_TECNICO':
      return 'PROJETO_TECNICO_CONCLUIDO';
    case 'PRODUCAO':
      return 'AGENDAR_MONTAGEM';
    case 'MONTAGEM':
      return 'MONTAGEM_FINALIZADA';
    default:
      return null;
  }
}

export function reversaoStatusClientePorSubetapa(params: {
  subetapa?: Subetapa | string | null;
}): { statusAplicado: string; statusAnterior: string } | null {
  const subetapa = String(params.subetapa || '').trim().toUpperCase();
...
export function classificarVendaPorFluxoMatrixOuLegado(params: {
  ...
  for (const agenda of params.agendas) {
    ...
    const subetapaFinalizacaoConcluida =
      SUBETAPAS_FINALIZACAO_VENDA.has(subetapa as Subetapa) &&
      execucao === 'CONCLUIDO';

    if (
      SUBETAPAS_PRODUCAO_ATIVA.has(subetapa as Subetapa) &&
      execucao !== 'CONCLUIDO'
    ) {
      emProducao = true;
    }

    if (subetapaFinalizacaoConcluida) {
      finalizada = true;
    }

    if (
      SUBETAPAS_OBRA_VIGENTE.has(subetapa as Subetapa) &&
      !subetapaFinalizacaoConcluida
    ) {
      obraVigente = true;
    }
  }
  ...
  return {
    emProducao,
    finalizada,
    obraVigente: !finalizada && obraVigente,
  };
}
...
const _PIPELINE_CLIENTE_FASES_DATA = [
  ...
  { ordem: 2, fase: 'MEDICAO_VENDA', label: 'Medição', cor: '#3b82f6', temAgendamento: true, verificacao: (c: any) => (c?.agendamentos_loja ?? []).length > 0 || (c?.agendamentos_fabrica ?? []).length > 0 },
