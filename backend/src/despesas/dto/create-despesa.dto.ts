...
export class CreateDespesaDto {
  ...
  @Matches(/^(\d{2}\/\d{2}\/\d{4}|\d{4}-\d{2}-\d{2})$/, {
    message: 'data_registro deve ser dd/mm/aaaa ou YYYY-MM-DD',
  })
  data_registro?: string;

  @IsString()
  @Matches(/^(\d{2}\/\d{2}\/\d{4}|\d{4}-\d{2}-\d{2})$/, {
    message: 'data_vencimento deve ser dd/mm/aaaa ou YYYY-MM-DD',
  })
  ...
  @IsString()
  @Matches(/^(\d{2}\/\d{2}\/\d{4}|\d{4}-\d{2}-\d{2})$/, {
    message: 'data_pagamento deve ser dd/mm/aaaa ou YYYY-MM-DD',
  })
