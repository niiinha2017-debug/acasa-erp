...
export function renderHeaderA4(
  doc: InstanceType<typeof PDFKitDoc>,
  empresa: any,
  title: string,
) {
  ...
  const endLinha = [logradouro, numero && `Nº ${numero}`, bairro]
    .filter(Boolean)
    .join(' - ');
  const cidadeLinha = [cep && `CEP: ${cep}`, cidade, uf]
    .filter(Boolean)
    .join(' - ');

  if (cnpj) {
    doc.text(`CNPJ: ${cnpj}`, tx, ty);
    ty += 12;
  }
  if (endLinha) {
    doc.text(`End: ${endLinha}`, tx, ty, { width: 200 });
    ty += 12;
  }
  if (cidadeLinha) {
    doc.text(cidadeLinha, tx, ty, { width: 200 });
    ty += 12;
  }
  if (telefone) {
    doc.text(`Fone: ${telefone}`, tx, ty);
    ty += 12;
  }
  if (email) {
    doc.text(`Email: ${email}`, tx, ty, { width: 200 });
    ty += 12;
  }
  ...
  doc
    .font('Helvetica')
    .fontSize(8)
    .text(`Gerado em: ${new Date().toLocaleString('pt-BR')}`, 0, 140, {
      align: 'center',
    });
