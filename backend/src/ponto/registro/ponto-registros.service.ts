...
export class PontoRegistrosService {
  ...
  async criar(dto: CriarPontoRegistroDto) {
    ...
    if (!funcionario_id) throw new BadRequestException('funcionario_id inválido')

    const data_hora = this.parseDataHora(dto.data_hora)
    const tipo = (String(dto.tipo || '').toUpperCase().trim() === 'SAIDA')
    ...
    return this.prisma.ponto_registros.create({
      ...
      select: { id: true, tipo: true, data_hora: true, status: true, observacao: true, funcionario_id: true },
    })
  }
  ...
  async atualizar(id: number, dto: AtualizarPontoRegistroDto) {
    ...
    const dataHora = dto.data_hora ? this.parseDataHora(dto.data_hora) : undefined
