...
export class PontoRelatorioService {
  ...
  private async gerarPdfPontoBuffer(payload: {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        let totalMetaMs = 0;
        const cargaDecimal = Number(payload.funcionario?.carga_horaria_dia || 0);
        ...
        for (let d = 1; d <= diasNoMes; d++) {
          ...
          for (let i = 0; i < diaRegs.length; i += 2) {
            if (diaRegs[i] && diaRegs[i+1] && diaRegs[i].tipo === 'ENTRADA' && diaRegs[i+1].tipo === 'SAIDA') {
        ...
        // 3. Tabela
        const col = { d: 35, e1: 95, s1: 140, e2: 185, s2: 230, t: 280, obs: 335 };
        let curY = resY + 60;
        const rowH = 14.3;
        ...
        for (let d = 1; d <= diasNoMes; d++) {
          ...
          doc.fillColor(diaSem === 0 ? '#94a3b8' : '#1e293b').font('Helvetica').fontSize(7);
          ...
          let dMs = 0;
          if (e1 && s1 && e1.tipo==='ENTRADA' && s1.tipo==='SAIDA') dMs += (new Date(s1.data_hora).getTime() - new Date(e1.data_hora).getTime());
          if (e2 && s2 && e2.tipo==='ENTRADA' && s2.tipo==='SAIDA') dMs += (new Date(s2.data_hora).getTime() - new Date(e2.data_hora).getTime());
          ...
          if (j) doc.fillColor('#e11d48').fontSize(6).text(j.tipo.toUpperCase(), col.obs, curY + 4);
          else if (diaSem === 0) doc.fillColor('#94a3b8').text('REPOUSO SEMANAL', col.obs, curY + 4);

          curY += rowH;
        }

        // 4. Assinaturas
        doc.strokeColor('#cbd5e1').lineWidth(0.5).moveTo(50, 760).lineTo(230, 760).stroke();
        doc.moveTo(350, 760).lineTo(530, 760).stroke();
        doc.fontSize(7).fillColor('#475569').text('ASSINATURA DO FUNCIONÁRIO', 50, 765, { width: 180, align: 'center' });
        doc.text('RESPONSÁVEL / EMPRESA', 350, 765, { width: 180, align: 'center' });
  ...
  async gerarPdfMensalESalvar(params: { funcionario_id: number; mes: number; ano: number }) {
    const { funcionario, registros, justificativas } = await this.relatorioMensalPdfData(params);
